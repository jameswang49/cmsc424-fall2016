...
def studentindex(request, student_id):
	today = timezone.now()
	context = { 'student_id': student_id, 'course_list': Student.objects.get(pk=student_id).courses.all(), 'a_list': Student.objects.get(pk=student_id).studentassignment_set, 'today': today }
